module TestIO
    ...
    io = open(filename)
    df = CSV.read(io, null="NA", weakrefstrings=false)
    @test isopen(io)
    close(io)
    ...
    @test isequal(df[1, 18], Nullable{String}())
    ...
    df1 = CSV.read("$data/separators/sample_data.csv")
    df2 = CSV.read("$data/separators/sample_data.tsv", delim='\t')
    df3 = CSV.read("$data/separators/sample_data.wsv", delim=' ')
    ...
    df = CSV.read("$data/newlines/os9.csv")
    @test isequal(CSV.read("$data/newlines/osx.csv"), df)
    @test isequal(CSV.read("$data/newlines/windows.csv"), df)

    @test isequal(df, CSV.read("$data/newlines/os9.csv"))
    @test isequal(df, CSV.read("$data/newlines/osx.csv"))
    @test isequal(df, CSV.read("$data/newlines/windows.csv"))

    #test_group("CSV.read treats rows as specified.")

    df1 = CSV.read("$data/skiplines/skipfront.csv", datarow = 4)
    df2 = CSV.read("$data/skiplines/skipfront_windows.csv", datarow = 4)

    @test isequal(df1, df2)

    #test_group("CSV.read handles custom delimiters.")
    df = CSV.read("$data/decimal/period.csv")
    @test isequal(df[2, :A], Nullable(0.3))
    @test isequal(df[2, :B], Nullable(4.0))

    #test_group("CSV.read column names.")
    ns = [:Var1, :Var2, :Var3, :Var4, :Var5]
    df = CSV.read("$data/typeinference/mixedtypes.csv")
    names!(df, ns)
    @test isequal(df, CSV.read("$data/typeinference/mixedtypes.csv", datarow=2, header = ns))

    df = CSV.read("$data/separators/sample_data.csv", datarow = 1, header = ns[1:3])
    ...
    df = CSV.read(filename, weakrefstrings=false)
    ...
    @test isa(df[:BoolColumn], NullableArray{String,1})
    @test isa(df[:StringColumn], NullableArray{String,1})

    filename = "$data/typeinference/mixedtypes.csv"
    df = CSV.read(filename, weakrefstrings=false)
    @test isa(df[:c1], NullableArray{String,1})
    ...
    @test isa(df[:c3], NullableArray{String,1})
    @test isequal(df[:c3][1], Nullable("0"))
    @test isequal(df[:c3][2], Nullable("1"))
    @test isequal(df[:c3][3], Nullable("f"))
    @test isa(df[:c4], NullableArray{String,1})
    @test isequal(df[:c4][1], Nullable("t"))
    @test isequal(df[:c4][2], Nullable("F"))
    @test isequal(df[:c4][3], Nullable("TRUE"))
    @test isa(df[:c5], NullableArray{String,1})
